...
![Alt text](Flow/CICD_Jen_K8_ArgoCD.png)

### Phase - 1 : Bootstrap Eksctl, K8 and ArgoCD setup
 - create a eks-bootstrap server with t2.micro,8gb ram and ubuntu 22.0 flavor #change hostname to 'EKS'if needed
 - create and assign ec2 - adminaccess and attach to the device
 - Run the Following commands sequentially or seperate each part as individual scripts and run them in sudo to save time
   ```sh
    sudo apt update
    sudo apt upgrade -y
   ...
   ```
### Phase - 2 : Install and Configure Jenkins-Master & Jenkins-Agent
 - Create 3 VM's in EC2 with ubuntu 22.04,t2.medium and 20GB Namely: Jenkins-Master,Jenkins-Agent,SonarQube
 ...
 - Install Java and Jenkins (on both servers) #refer [Official documentaion](https://www.jenkins.io/doc/book/installing/linux/ ) for commands (on both servers)
  ```sh
  sudo apt update
  sudo apt upgrade -y
  ...
  ```
 - Steps to be performed for Jenkins Master-Server Architecture.
  ```sh
  In jenkins management console and installing and adding plugins
  ...
  6. #adding github creds to jenkins
      Manage Jenkins-> credentials -> username=githubusername; password= personal access token from github site; ID= github-creds github-token
  7. #Docker plugin installation and configuration
     ...
     Manage jenkins -> Credentials -> Global -> Add Creds- -> username - docker username ; token - docker-token ;id-docker-creds
     #Note without restarting jenkins server docker login will fail 
  ```

### Phase - 3 : Install and configure Postgres, Sonarqube and Trivy
 - Perform these on the SonarQube Server
  ...
  #From sonarqube console create a token for jenkins Administrator->My account->security ->genrate Token #sample Token- sqa_55be8ce73a6f6a30ca21b509b9af2140e443d977
  ```
 - Perform these steps to add sonarqube token in Jenkins and to create a CI Job
  ```sh
  ...
  #also upload the necessary source binaries in the repo and trigger the job from Jenkins #the files can be taken from - https://github.com/Velkaandhans/CICD_Jen_K8_ArgoCD.git
  ```
 - With this the CI part of the job is done     
 - Perform the following to create the CD Part
  ```sh
  #ArgoCD and Github integration
  #Create a new repo in github consisting of the manifest files #refer https://github.com/Velkaandhans/CICD_Jen_K8_ArgoCD_CI_Manifest-files.git for manifest files
...
### Phase - 4 : Integrating CI and CD part
 - In Jenkins management console, create a New CD Job as a Pipeline
  ```sh
...
admin->configure->API token->JENKINS_API_TOKEN: 11c89c13908bb03a0cbf3820528b6fda36
...
stage("Trigger CD Pipeline") {
  ...
  ```
 - The CICD Pipeline is done and any new changes will auto trigger the CI and CD Job which will reflect in the URL.

 - Delete the eks cluster Once the project is done. `eksctl delete cluster --region=ap-south-1 --name=eks-cluster`
